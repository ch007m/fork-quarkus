...
----
<1> Quarkus kubernetes client property of the KubeConfig YAML string populated by the kind test container

For the folks interested to develop a new `DevService` extension able to rely on a kind container in order to customize the cluster and to deploy by example: Argocd, Tekton, Certificate Manager, Keycloak etc. then you can create an extension project, add the following dependency to your `deployment` maven module:
...
create a DevService `Processor` class where you include the parameters: `KubernetesDevServiceRequestBuildItem` and `KubernetesDevServiceInfoBuildItem` within the body of the method which has been annotated with `@BuildStep`

.DevService Extension using Kube
[source,java]
----
...
public class ArgocdExtensionProcessor {
    ...
    public void deployArgocd(
            Optional<KubernetesDevServiceInfoBuildItem> kubeServiceInfo, <1>
            Optional<KubernetesDevServiceRequestBuildItem> kubeInfo, <2>
            BuildProducer<DevServicesResultBuildItem> devServicesResultBuildItem) {

        if (!Arrays.asList("kind", "k3s").contains(kubeInfo.get().getFlavor())) {
        // The kindContainer started is an Api Server and not: kind, k3s
        throw new RuntimeException("The test container started is an Api Server and not a cluster: kind or k3s !");
...
----
<1> This `KubernetesDevServiceInfoBuildItem` object which has been instantiated by the `quarkus-kubernetes-client` will be filled with the cluster `containerId` and the kubeConfig YAML file of the container.
<1> The `KubernetesDevServiceRequestBuildItem` provides as information the type of the cluster created by the kind test container: kind, k3 or api_only.

Then, you will be able to configure the Fabric8 Kubernetes Client to access the cluster, to tailor the operations that you will perform using the flavor (kind, k3, api).
